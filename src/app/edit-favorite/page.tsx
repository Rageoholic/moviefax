			...
			<div className="flex w-full max-w-lg flex-col gap-8 rounded-3xl border border-white/10 bg-white/5 p-8 shadow-2xl shadow-black/25 backdrop-blur">
				...
				<form className="flex flex-col gap-4" method="GET">
					<label
						...
						htmlFor="query"
					>
						Search for a new favorite movie
						<input
					...
					<div className="flex gap-3">
						...
						<Link
							className="rounded-full border border-white/20 px-4 py-2 text-center font-medium text-sm text-white/80 transition hover:bg-white/10"
							href="/dashboard"
						>
							Cancel
						</Link>
				...
				{query ? (
					<form action={updateFavoriteMovie} className="flex flex-col gap-4">
						<label
							className="flex flex-col gap-2 text-sm text-white/80"
